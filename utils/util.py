...
#         # top_down_words = []
#         # top_down_boxes = []
#         # words_boxes_zip = sorted(list(zip(each_words, each_boxes)), key=lambda x: (x[1][1], x[1][0]))
#         # y_value = words_boxes_zip[0][1][1]
#         # tmp_list = []
#         # for word, box in words_boxes_zip:
#         #     if abs(y_value-box[1]) <= abs(box[3]-box[1])*0.08:
#         #         tmp_list.append([word, box])
#         #     else:
#         #         sort_tmp = sorted(tmp_list, key=lambda x: x[1][0])
#         #         for w, b in sort_tmp:
#         #             top_down_words.append(w)
#         #             top_down_boxes.append(b)

#         #         y_value = box[1]
#         #         tmp_list = [[word, box]]

#         # batch_words.append(top_down_words)
#         # batch_boxes.append(top_down_boxes)
...
#         words_example = [word.lower() for word in words[batch_index]] # for문 밖으로 빼는게 더 효율적일듯..! for문 돌때마다 똑같은 words를 소문자로 변경함
...
#         # start_positions = start_positions[:batch_index+1]
#         # end_positions = end_positions[:batch_index+1]
